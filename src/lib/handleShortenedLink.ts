...
export async function handleShortenedLink(
  ...
  if (
    ...
    const url = `${Config.shortenerApiHost}/convert/1688/${link.pathname.slice(
      3
    )}`;
    const response: ApiResponse<{ url: string }> = await fetchData(url);
    if (response && response.data) {
    ...
    const url = `${Config.shortenerApiHost}/convert/kyoushop${link.pathname}`;
    const response: ApiResponse<{ url: string }> = await fetchData(url);
    if (response && response.data) {
      return new URL(response.data.url);
    }
  } else if (link.hostname === 'm.tb.cn') {
    const url = `${Config.shortenerApiHost}/convert/taobao${link.pathname}`;
    const response: ApiResponse<{ url: string }> = await fetchData(url);
    if (response && response.data) {
      return new URL(response.data.url);
    }
